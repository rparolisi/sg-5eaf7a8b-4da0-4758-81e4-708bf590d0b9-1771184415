...
import {
    ...
    Plus, Trash2, Layers, X, ChevronRight
} from 'lucide-react';
...
// Interfaccia per le righe della tabella (Dati, Header Gruppo, Footer Gruppo)
type TableRow =
...
const INITIAL_COLUMNS: ColumnDef[] = [
    ...
    { id: 'person', label: 'Person', visible: false, width: 100, type: 'text', align: 'left' }, // Campo nascosto utile per grouping
];
...
const SettingsModal = ({
    ...
    // Helpers per modifica settings locale
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl h-[600px] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-white">
                    {activeTab === 'columns' && (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            {localSettings.columns.map(col => (
                                <label key={col.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${col.visible ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                        ...
                        <div className="space-y-4">
                            {localSettings.filters.map((filter) => (
                                <div key={filter.id} className="flex flex-col md:flex-row gap-3 items-start md:items-center bg-gray-50 p-3 rounded-lg border border-gray-200">
                                    <select className="w-24 p-2 border border-gray-300 rounded text-sm font-medium" value={filter.type} onChange={(e) => updateFilter(filter.id, 'type', e.target.value)}>
                                        <option value="include">Include</option>
                                        <option value="exclude">Exclude</option>
                                    </select>
                                    <select className="flex-1 p-2 border border-gray-300 rounded text-sm" value={filter.columnId} onChange={(e) => updateFilter(filter.id, 'columnId', e.target.value)}>
                                        {allColumns.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
                                    </select>
                                    <select className="w-32 p-2 border border-gray-300 rounded text-sm" value={filter.operator} onChange={(e) => updateFilter(filter.id, 'operator', e.target.value)}>
                                    ...
                                    {filter.operator === 'between' ? (
                                        <div className="flex gap-2 w-full md:w-auto">
                                            <input type="text" className="w-24 p-2 border border-gray-300 rounded text-sm" placeholder="From" value={filter.value} onChange={(e) => updateFilter(filter.id, 'value', e.target.value)} />
                                            <input type="text" className="w-24 p-2 border border-gray-300 rounded text-sm" placeholder="To" value={filter.value2 || ''} onChange={(e) => updateFilter(filter.id, 'value2', e.target.value)} />
                                        </div>
                                    ) : (
                                        <input type="text" className="flex-1 p-2 border border-gray-300 rounded text-sm min-w-[150px]" placeholder="Value..." value={filter.value} onChange={(e) => updateFilter(filter.id, 'value', e.target.value)} />
                                    )}
                    ...
                    {activeTab === 'group' && (
                        <div className="space-y-4">
                            <p className="text-xs text-gray-500 italic">Drag columns to group. The order determines the nesting level.</p>
                            {localSettings.groups.map((group, idx) => (
                                <div key={group.id} className="flex gap-3 items-center bg-gray-50 p-3 rounded-lg border border-gray-200">
                                    ...
                                    <select className="flex-1 p-2 border border-gray-300 rounded text-sm" value={group.columnId} onChange={(e) => updateGroup(group.id, e.target.value)}>
                                        {allColumns.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
                                    </select>
...
export default function PortfolioValuation() {
    ...
    const basePortfolioData = useMemo(() => {
        ...
        // Qui facciamo una prima aggregazione "piatta" per Ticker+Persona (se serve) o solo Ticker.
        // SAP Grouping solitamente parte dai dati atomici o semi-aggregati.
        // Manteniamo la logica attuale che aggrega per Ticker (e basta), ma se vogliamo raggruppare per Persona,
        // dovremmo aggregare per (Ticker, Person).
        // Per semplicità, la tabella attuale è "Per Ticker". Se l'utente vuole raggruppare per Person,
        // dovremmo cambiare la logica di aggregazione iniziale.
        // SOLUZIONE: Aggreghiamo per una chiave composita che include TUTTI i campi potenzialmente raggruppabili.
        // In questo caso: Ticker e Persona.

        const groups: Record<string, any> = {};

        filtered.forEach(t => {
            // Chiave composita per mantenere granularità
        ...
        const items = Object.values(groups).map(g => {
            ...
            const total_dividends = pyData ? pyData.dividends : null; // Nota: questo è dividendo unitario o totale per ticker?
            // Se pyData.dividends è il totale storico del ticker, va riproporzionato per persona?
            // Python API attuale ritorna dividendi totali PER QUELLA PERSONA se passiamo user_id.
            // Assumiamo che pythonData sia corretto.
            ...
            return {
                ...
                ticker: g.ticker,
                y_ticker: g.ticker, // o map se disponibile
                person: g.person,
    ...
    const processedTableData = useMemo(() => {
        ...
        if (viewSettings.filters.length > 0) {
            data = data.filter(item => {
                // Deve soddisfare TUTTI i filtri (AND logic per semplicità, oppure si può fare complessa)
        ...
        // Se ci sono gruppi, dobbiamo prima ordinare per le colonne di gruppo
        const groupCols = viewSettings.groups.map(g => g.columnId);

        // 3. ORDINAMENTO (Sorting)
        // Priorità: Colonne di Gruppo > Regole di Sort Utente
        const sortRules = [
            ...groupCols.map(col => ({ columnId: col, direction: 'asc' as const })), // I gruppi sono sempre ordinati
            ...viewSettings.sorts
        ];
        ...
        if (sortRules.length > 0) {
            ...
            // Default sort se nulla è specificato
            data.sort((a, b) => b.total_exposure - a.total_exposure);
        }

        // 4. STRUTTURAZIONE DATI (Flat vs Grouped)
        if (groupCols.length === 0) {
            // Nessun gruppo: restituisci righe dati semplici
            return data.map(item => ({ type: 'data', data: item } as TableRow));
        } else {
            // Raggruppamento Attivo
            const resultRows: TableRow[] = [];

            // Funzione ricorsiva o iterativa per inserire header e footer
            // Facciamo iterativa per semplicità su 1 livello, ma il codice supporta N
            // Usiamo un approccio di "Change Detection" sui valori delle chiavi di gruppo

            // Calcolo totali complessivi
            // ... (omesso per brevità, focalizziamoci sulla struttura visuale)

            let previousValues: Record<string, any> = {};
            const groupAggregates: Record<string, any> = {}; // Per i subtotali (non implementato full qui per brevità)
            ...
            data.forEach((item, index) => {
                ...
                groupCols.forEach((groupCol, level) => {
                    ...
                    if (currentVal !== prevVal) {
                        // Se cambia, inserisci Header
                        // (In un'implementazione completa dovremmo chiudere i footer dei gruppi precedenti)
                        resultRows.push({
                            type: 'group_header',
                            key: groupCol,
                            value: currentVal,
                        ...
                        // Reset chiavi livelli successivi
    ...
    }, [basePortfolioData, viewSettings]);

    // --- CALCOLO TOTALI GLOBALI ---
    const totals = useMemo(() => {
        // Calcoliamo i totali sui dati FILTRATI (ma non raggruppati strutturalmente)
        // Dobbiamo estrarre solo i 'data' rows da processedTableData
                        ...
                        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden relative min-h-[400px]">
                            ...
                            <div className="overflow-x-auto min-h-[400px]">
                                <table className="w-full text-sm text-left border-collapse">
                                                    ...
                                                    return (
                                                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                                            {visibleColumns.map(col => {
                                                                ...
                                                                } else if (col.id === 'performance_perc') { // Fix return % id
                                                                    content = (
                                    ...
                                    {/* FOOTER TOTALI (Visibile solo se non stiamo raggruppando o se vuoi totale generale) */}
