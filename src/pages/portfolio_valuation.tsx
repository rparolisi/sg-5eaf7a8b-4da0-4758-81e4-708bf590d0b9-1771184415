...
export default function PortfolioPlotPage() {
    ...
    // --- NUOVI STATI PER RLS E MULTI-CURRENCY ---
    ...
    // 1. Fetch Iniziale Transazioni & Setup Sessione
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            try {
                // A. Recupera Sessione e Preferenze Valuta (NUOVO)
                ...
                // B. Recupera Transazioni (Supabase RLS filtra automaticamente)
    ...
    const handlePlot = async () => {
        if (!userId) {
            alert("Devi essere loggato per calcolare i dati.");
            return;
        }
        ...
        try {
            ...
            const pyRes = await fetch(`${PYTHON_API_BASE_URL}/api/portfolio_history`, {
                ...
                body: JSON.stringify({
                    user_id: userId, // --- FIX: Passaggio user_id per RLS ---
    ...
    // Helper per simbolo valuta (es. USD -> $)
